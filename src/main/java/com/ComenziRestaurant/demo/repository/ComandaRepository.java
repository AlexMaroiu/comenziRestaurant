...
public interface ComandaRepository extends JpaRepository<Comanda, Integer> {
    ...
    @Query(value = ("select id, sum(portii) as portii, username, id_mancare, sum(pret) as pret, finalizat"+
            "  from Comanda where (username=?1 and finalizat = false) group by  id_mancare"),
    nativeQuery = true)
